...
for root, dirs, fs in os.walk(IMG_PATH):
    for f in fs:
        ...
        img = dil_ero(img)
        img = de_binary(img)
        save_name = 'BK_thin_%s_%02s.jpg' % (root[-1], f[2:4])
        print(save_name)
        make_dir(SAVE_PATH + '/' + 'BK_thin')
        make_dir(SAVE_PATH + '/' + 'BK_thin' + '/' + root[-1])
        cv2.imencode('.jpg', img)[1].tofile('result/BK_thin' + '/' + root[-1] + '/' + save_name)
