...
def quadratic_bezier(t, b_points):
    ...
    x = (1 - t) ** 2 * b_points[0, 0] + 2 * (1 - t) * \
        t * b_points[1, 0] + t ** 2 * b_points[2, 0]
    y = (1 - t) ** 2 * b_points[0, 1] + 2 * (1 - t) * \
        t * b_points[1, 1] + t ** 2 * b_points[2, 1]
    return x, y
...
def cut_and_connect(img: np.ndarray, co_linear, parallel, same_sub_correspond, no_in_class, real_C, order, all_point):
    ...
    for i in range(len(same_sub_correspond)):
        # 两个點的坐標
        point1 = (real_C[same_sub_correspond[i][0]][1],
                  real_C[same_sub_correspond[i][0]][0])
        point2 = (real_C[same_sub_correspond[i][1]][1],
                  real_C[same_sub_correspond[i][1]][0])
        # 在圖像上繪製線段
    ...
    if len(parallel) != 0:
        ...
        for i in range(len(parallel)):
            ...
            cv2.line(cut_img, point1, point2, (0, 0, 0), 2)
        # 連完線後做切割
        img, co_linear, order = sub_contour_cut_parallel(
            cut_img, co_linear, parallel, real_C, order)
    ...
    if len(co_linear) != 0:
        ...
        for i in range(len(co_linear)):
            ...
            if co_linear[i][0] != -1 and co_linear[i][1] != -1:
                # 两个點的坐標
                point1 = (real_C[co_linear[i][0]][1],
                          real_C[co_linear[i][0]][0])
                point2 = (real_C[co_linear[i][1]][1],
                          real_C[co_linear[i][1]][0])
                # 在圖像上繪製線段
                cv2.line(cut_img, point1, point2, (0, 0, 0), 2)
    ...
    if len(no_in_class) != 0:
        ...
        for i in range(len(no_in_class)):
            cut_x, cut_y = calculate_tangent_line(
                all_point, no_in_class[i], real_C)
            # 两个點的坐標
...
def Douglas_Peucker(points_set):
    ...
    image = cv2.drawContours(
        image, [approx], contourIdx=0, color=(255, 255, 255), thickness=2)
